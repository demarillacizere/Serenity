import React from "react";

type Props = {};

export function WhySection() {
  return (
    <div className="container my-24 mx-auto md:px-6 text-gray-800">
      <section className="mb-32 text-center">
        <h2 className="mb-16 text-3xl font-bold">
          Why join<u className="text-primary-orange "> Serenity?</u>
        </h2>
        <div className="grid gap-x-6 md:grid-cols-3 lg:gap-x-12">
          <div className="mb-12 md:mb-0">
            <div className="mb-6 inline-block rounded-md bg-primary-100 p-4 text-primary">
              <svg
                fill="#616161"
                height="48px"
                width="48px"
                version="1.1"
                id="Capa_1"
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 490 490"
                stroke="#616161"
              >
                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                <g
                  id="SVGRepo_tracerCarrier"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                ></g>
                <g id="SVGRepo_iconCarrier">
                  {" "}
                  <g>
                    {" "}
                    <path d="M245,208.977c36.565,0,66.207-29.642,66.207-66.207S281.565,76.563,245,76.563s-66.208,29.642-66.208,66.207 S208.435,208.977,245,208.977z M245,91.875c28.063,0,50.895,22.831,50.895,50.895c0,28.063-22.831,50.895-50.895,50.895 c-28.064,0-50.896-22.831-50.896-50.895C194.104,114.706,216.936,91.875,245,91.875z"></path>{" "}
                    <path d="M110.796,286.815c0,61.734,0,76.049,0,76.049s130.622,0,131.072,0l-63.971-138.678 C177.898,224.186,110.796,225.082,110.796,286.815z M126.108,347.552v-60.737c0-34.228,27.182-43.702,42.448-46.323l49.385,107.06 H126.108z"></path>{" "}
                    <path d="M312.102,224.186l-63.971,138.678h131.073c0,0,0-14.315,0-76.049C379.204,225.082,312.102,224.186,312.102,224.186z M363.892,347.552h-91.834l49.422-107.137c15.242,2.507,42.412,11.825,42.412,46.4V347.552z"></path>{" "}
                    <polygon points="244.999,224.186 198.922,224.186 244.999,326.181 291.076,224.186 "></polygon>{" "}
                    <path d="M245,0C109.689,0,0,109.69,0,245s109.689,245,245,245c135.31,0,245-109.69,245-245S380.31,0,245,0z M245,474.688 C118.349,474.688,15.313,371.65,15.313,245C15.313,118.349,118.349,15.313,245,15.313c126.65,0,229.688,103.037,229.688,229.688 C474.688,371.65,371.65,474.688,245,474.688z"></path>{" "}
                  </g>{" "}
                </g>
              </svg>
            </div>
            <h5 className="mb-4 text-lg font-bold">Professional Care</h5>
            <p className=" ">
              Laudantium totam quas cumque pariatur at doloremque hic quos quia
              eius. Reiciendis optio minus mollitia rerum labore facilis
              inventore voluptatem ad, quae quia sint. Ullam.
            </p>
          </div>

          <div className="mb-12 md:mb-0">
            <div className="mb-6 inline-block rounded-md bg-primary-100 p-4 text-primary">
              <svg
                height="48px"
                width="48px"
                version="1.1"
                id="Layer_1"
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 392.663 392.663"
                fill="#616161"
              >
                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                <g
                  id="SVGRepo_tracerCarrier"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                ></g>
                <g id="SVGRepo_iconCarrier">
                  {" "}
                  <path
                    fill="white"
                    d="M275.297,56.372c0-19.071-15.515-34.586-34.586-34.586c-16.097,0-29.996,10.925-33.681,26.505 c-2.392,9.762-17.131,12.735-21.269,0c-3.685-15.58-17.519-26.505-33.681-26.505c-19.071,0-34.651,15.515-34.651,34.586 c0,39.305,52.299,84.558,78.869,103.37C222.998,140.929,275.297,95.806,275.297,56.372z"
                  ></path>{" "}
                  <path d="M240.711,0c-17.778,0-33.875,8.21-44.283,21.333C186.02,8.21,169.923,0,152.145,0 c-31.16,0-56.436,25.341-56.436,56.436c0,63.16,90.893,123.152,94.707,125.737c3.62,2.263,8.598,2.133,11.895,0 c3.879-2.521,94.707-62.578,94.707-125.737C297.082,25.341,271.87,0,240.711,0z M117.559,56.372 c0-19.071,15.515-34.586,34.651-34.586c16.097,0,29.996,10.925,33.681,26.505c4.073,12.735,18.877,9.762,21.269,0 c3.685-15.58,17.519-26.505,33.681-26.505c19.071,0,34.586,15.515,34.586,34.586c0,39.305-52.299,84.558-78.869,103.37 C169.858,140.929,117.559,95.806,117.559,56.372z"></path>{" "}
                  <path
                    fill="white"
                    d="M368.064,270.158c1.875-1.939,2.844-4.396,2.844-7.111v-135.37c0-9.956-8.145-18.101-18.101-18.101 s-18.101,8.145-18.101,18.101v53.139c6.465,6.982,10.537,16.291,10.537,26.505v44.671c0,6.012-4.848,10.925-10.925,10.925 c-6.012,0-10.925-4.848-10.925-10.925v-44.671c0-9.374-7.628-17.002-17.002-17.002c-9.374,0-17.002,7.628-17.002,17.002v43.378 c0,2.715-1.034,5.301-2.844,7.37l-38.788,42.602c-7.046,7.758-10.925,17.778-10.925,28.25v31.547 c0,5.624,4.655,10.279,10.279,10.279h48.937c5.624,0,10.279-4.655,10.279-10.279v-12.929c0-8.275,3.168-16.162,8.857-22.174 L368.064,270.158z"
                  ></path>{" "}
                  <path
                    fill="#23981b"
                    d="M352.808,87.79c-22.044,0-39.887,17.907-39.887,39.887v41.309c-2.069-0.388-4.331-0.517-6.594-0.517 c-21.398,0-38.788,17.39-38.788,38.788v39.111l-35.943,39.564c-10.667,11.766-16.614,27.022-16.614,42.99v31.547 c0,17.713,14.352,32.065,32.065,32.065h49.002c17.713,0,32.065-14.352,32.065-32.065v-12.929c0-2.65,1.034-5.172,2.844-7.111 l52.752-55.337c5.689-6.012,8.857-13.834,8.857-22.174v-135.24C392.695,105.632,374.788,87.79,352.808,87.79z M306.456,360.469 c0,5.624-4.655,10.279-10.279,10.279h-49.002c-5.624,0-10.279-4.655-10.279-10.279v-31.612c0-10.537,3.879-20.558,10.925-28.25 l38.723-42.602c1.875-2.004,2.844-4.655,2.844-7.37v-43.378c0-9.374,7.628-17.002,17.002-17.002s17.002,7.628,17.002,17.002v44.671 c0,6.012,4.848,10.925,10.925,10.925c6.012,0,10.925-4.848,10.925-10.925v-44.671c0-10.279-4.008-19.523-10.537-26.505v-53.01 c0-9.956,8.145-18.101,18.101-18.101s18.101,8.145,18.101,18.101v135.37c0,2.651-1.034,5.172-2.844,7.111l-52.881,55.337 c-5.689,6.012-8.857,13.834-8.857,22.174v12.735H306.456z"
                  ></path>{" "}
                  <path
                    fill="white"
                    d="M106.311,258.004c-1.875-2.004-2.844-4.655-2.844-7.37v-43.378c0-9.374-7.628-17.002-17.002-17.002 s-17.002,7.628-17.002,17.002v44.671c0,6.012-4.848,10.925-10.925,10.925s-10.925-4.849-10.925-10.925v-44.671 c0-10.279,4.008-19.523,10.537-26.505v-53.01c0-9.956-8.145-18.101-18.101-18.101s-18.101,8.145-18.101,18.101v135.37 c0,2.651,1.034,5.172,2.844,7.111l52.752,55.337c5.689,6.012,8.857,13.834,8.857,22.174v12.929c0,5.624,4.655,10.279,10.279,10.279 h49.002c5.624,0,10.279-4.655,10.279-10.279V329.05c0-10.537-3.879-20.558-10.925-28.251L106.311,258.004z"
                  ></path>{" "}
                  <path
                    fill="#23981b;"
                    d="M161.196,285.931l-35.879-39.499v-39.111c0-21.398-17.39-38.788-38.788-38.788 c-2.263,0-4.396,0.129-6.594,0.517v-41.309c0-22.044-17.907-39.887-39.887-39.887C18.004,87.79,0.097,105.632,0.097,127.677v135.37 c0,8.275,3.168,16.162,8.857,22.174l52.752,55.337c1.875,1.939,2.844,4.396,2.844,7.111v12.929 c0,17.713,14.352,32.065,32.065,32.065h49.002c17.713,0,32.065-14.352,32.065-32.065v-31.612 C177.81,312.954,171.862,297.697,161.196,285.931z M155.959,360.469c0,5.624-4.655,10.279-10.279,10.279H96.743 c-5.624,0-10.279-4.655-10.279-10.279v-12.929c0-8.275-3.168-16.162-8.857-22.174l-52.752-55.337 c-1.875-1.939-2.844-4.396-2.844-7.111v-135.24c0-9.956,8.145-18.101,18.101-18.101s18.101,8.145,18.101,18.101v53.139 c-6.465,6.982-10.537,16.291-10.537,26.505v44.671c0,6.012,4.848,10.925,10.925,10.925s10.925-4.848,10.925-10.925v-44.671 c0-9.374,7.628-17.002,17.002-17.002s17.002,7.628,17.002,17.002v43.378c0,2.715,1.034,5.301,2.844,7.37l38.723,42.602 c7.046,7.758,10.925,17.778,10.925,28.25v31.547H155.959z"
                  ></path>{" "}
                </g>
              </svg>
            </div>
            <h5 className="mb-4 text-lg font-bold">Prompt Care 24/7</h5>
            <p>
              Eum nostrum fugit numquam, voluptates veniam neque quibusdam ullam
              aspernatur odio soluta, quisquam dolore animi mollitia a omnis
              praesentium, expedita nobis!
            </p>
          </div>

          <div className="mb-12 md:mb-0">
            <div className="mb-6 inline-block rounded-md bg-primary-100 p-4 text-primary">
              <svg
                fill="#666666"
                version="1.1"
                id="Layer_1"
                xmlns="http://www.w3.org/2000/svg"
                height="48px"
                width="48px"
                viewBox="0 0 256 240"
                enable-background="new 0 0 256 240"
                stroke="#666666"
              >
                <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                <g
                  id="SVGRepo_tracerCarrier"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                ></g>
                <g id="SVGRepo_iconCarrier">
                  {" "}
                  <path d="M127.826,39.584c10.308,0,18.7-8.392,18.7-18.7s-8.392-18.7-18.7-18.7s-18.7,8.392-18.7,18.7S117.518,39.584,127.826,39.584 z M26.21,39.584c-10.308,0-18.7-8.392-18.7-18.7s8.392-18.7,18.7-18.7s18.7,8.392,18.7,18.7S36.518,39.584,26.21,39.584z M229.79,39.584c10.308,0,18.7-8.392,18.7-18.7s-8.392-18.7-18.7-18.7c-10.308,0-18.7,8.392-18.7,18.7S219.482,39.584,229.79,39.584 z M253.966,130.048c0,3.167-4.598,95.372-4.598,95.372c0,6.998-5.398,12.396-12.396,12.396c-6.998,0-12.396-5.398-12.396-12.396 c0,0-8.617-95.972-10.995-131.806l-19.741,23.724c-1.694,2.035-4.194,3.192-6.808,3.192c-0.339,0-0.68-0.019-1.021-0.059 c-2.972-0.345-5.569-2.165-6.905-4.842l-23.665-47.388v156.056c0,7.435-5.504,13.517-12.359,13.517 c-6.855,0-12.359-6.082-12.359-13.517V138.85c0-1.352-1.159-2.511-2.511-2.511c-1.352,0-2.511,1.159-2.511,2.511v85.448 c0,7.435-5.504,13.517-12.359,13.517c-6.855,0-12.359-6.082-12.359-13.517V67.387l-24.092,48.243 c-1.336,2.677-3.933,4.497-6.904,4.842c-0.341,0.039-0.682,0.059-1.021,0.059c-2.613,0-5.114-1.157-6.808-3.192L42.419,93.614 c-2.378,35.834-10.995,131.805-10.995,131.805c0,6.998-5.398,12.396-12.396,12.396s-12.396-5.398-12.396-12.396 c0,0-4.598-92.204-4.598-95.371c0,0-0.034-71.339-0.034-71.57c0-7.97,6.091-13.605,13.605-13.605c5.692,0,10.073,1.924,14.649,6.516 c0.131,0.132,36.851,44.193,36.851,44.193c-0.062-0.074,16.261-33.095,19.507-39.002c4.344-7.903,10.612-11.71,18.6-11.765 c0.019,0,0.041-0.005,0.059-0.005c0,0,45.073,0.012,45.348,0.022c8.069-0.022,14.396,3.788,18.772,11.752 c0.091,0.157,19.506,38.998,19.506,38.998s36.714-44.061,36.854-44.196c4.473-4.689,9.55-6.513,14.645-6.513 c7.514,0,13.605,6.091,13.605,13.605C254,61.212,253.966,130.048,253.966,130.048z"></path>{" "}
                </g>
              </svg>
            </div>
            <h5 className="mb-4 text-lg font-bold">A Supportive Community</h5>
            <p className=" ">
              Enim cupiditate, minus nulla dolor cumque iure eveniet facere
              ullam beatae hic voluptatibus dolores exercitationem? Facilis
              debitis aspernatur amet nisi?
            </p>
          </div>
        </div>
      </section>
    </div>
  );
}
